...
// ============================================================
//  wifi_config.h  –  Dành cho ESP8266
//  Thứ tự ưu tiên:
...
#include <ESP8266WiFi.h>       // ESP8266 – khác ESP32
#include <ESP8266WebServer.h>  // ESP8266 – khác ESP32
#include <DNSServer.h>
#include <EEPROM.h>

// ---------- Cấu hình AP ----------
#define WIFI_AP_SSID      "ESP8266_Config"
...
//  Lưu ý ESP8266: WiFi.config() chỉ có 4 tham số, không có secondaryDNS
// ============================================================
...
// ============================================================
//  HTML Captive Portal (dùng chung – giống bản ESP32)
// ============================================================
static const char configPage[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="UTF-8">
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h2 { font-size: 1.3rem; margin-bottom: 6px; color: #00d4ff; }
  p.sub { font-size: 0.8rem; color: rgba(255,255,255,0.5); margin-bottom: 20px; }
  label { display: block; font-size: 0.78rem; color: rgba(255,255,255,0.7); margin-bottom: 4px; }
  ...
  button {
    ...
    font-size: 0.95rem; font-weight: 600; cursor: pointer; transition: opacity .2s;
  }
  button:disabled { opacity: 0.5; cursor: not-allowed; }
  button:hover:not(:disabled) { opacity: 0.85; }
  #status { margin-top: 14px; font-size: 0.8rem; color: rgba(255,255,255,0.55); min-height: 18px; text-align: center; }
  .modal-overlay {
  ...
  .modal {
    background: #162032; border: 1px solid #00d4ff; border-radius: 14px;
    padding: 28px 24px; max-width: 280px; text-align: center; color: #fff;
  ...
  .ip-highlight {
    display: inline-block; margin: 10px 0; padding: 6px 16px;
  ...
  .modal button { margin-top: 16px; width: auto; padding: 9px 28px; font-size: 0.9rem; }
</style>
</head>
<body>
<div class="card">
  ...
  <select id="ssidSelect"><option value="">⏳ Đang quét mạng...</option></select>
...
<div class="modal-overlay" id="confirmModal">
  <div class="modal">
    ...
    <p>Sau khi thiết bị kết nối Wi-Fi, bạn có thể truy cập bảng điều khiển tại:</p>
...
function loadWiFiList() {
  ...
  fetch('/scanwifi')
    ...
    .then(list => {
      ...
      document.getElementById('status').textContent = 'Tìm thấy ' + list.length + ' mạng. Chọn và nhập mật khẩu.';
    })
    .catch(() => {
...
function showConfirmModal() {
  if (!document.getElementById('ssidSelect').value) {
...
function submitWiFi() {
  ...
  fetch('/savewifi?ssid=' + encodeURIComponent(ssid) + '&pass=' + encodeURIComponent(pass))
    ...
    .catch(() => { document.getElementById('status').textContent = '✅ Đã gửi! Thiết bị đang khởi động lại...'; });
}
loadWiFiList();
</script>
</body>
...
// ============================================================
static void wifi_startAP(ESP8266WebServer& server) {
  _apMode = true;
  ...
  auto sendPortal = [&server]() { server.send_P(200, "text/html", configPage); };
  ...
  server.on("/scanwifi", [&server]() {
    // ESP8266: scanNetworks() chỉ có 1 tham số, không hỗ trợ include hidden
  ...
  server.on("/savewifi", [&server]() {
    if (server.hasArg("ssid") && server.hasArg("pass")) {
      wifi_saveEEPROM(server.arg("ssid"), server.arg("pass"));
      server.send(200, "text/plain",
                  "✅ Đã lưu Wi-Fi \"" + server.arg("ssid") + "\". Đang khởi động lại...");
      delay(1500);
      ESP.restart();
    } else {
...
// ============================================================
//  Gọi trong loop() để xử lý DNS khi AP mode
// ============================================================
